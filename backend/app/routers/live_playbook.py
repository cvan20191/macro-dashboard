...
from app.schemas.fed_liquidity_overview import FedLiquidityOverviewResponse
...
from app.services.ingestion.fed_liquidity_cache import get_fed_liquidity_overview
...
@router.get("/macro-expectations", response_model=MacroExpectationsState)
async def macro_expectations(
    force_refresh: bool = Query(default=False, description="Bypass TTL file caches"),
) -> MacroExpectationsState:
    """
    Macro expectations / event prep overlay (calendar, FedWatch, NY Fed context).
    """
    try:
        snap_resp = await get_live_snapshot(force_refresh=force_refresh)
        state = build_dashboard_state(snap_resp.snapshot)
        ...
        logger.error("macro_expectations failed: %s", exc)
        raise HTTPException(status_code=503, detail=f"Macro expectations unavailable: {exc}") from exc


@router.get("/fed-liquidity-overview", response_model=FedLiquidityOverviewResponse)
async def fed_liquidity_overview(
    force_refresh: bool = Query(default=False, description="Bypass cache and re-fetch FRED history window"),
) -> FedLiquidityOverviewResponse:
    """
    Return chart-ready Fed balance sheet and policy-rate history from incremental cache.
    """
    try:
        return FedLiquidityOverviewResponse(**get_fed_liquidity_overview(force_refresh=force_refresh))
    except Exception as exc:
        logger.error("fed_liquidity_overview failed: %s", exc)
        raise HTTPException(status_code=503, detail=f"Fed liquidity overview unavailable: {exc}") from exc
