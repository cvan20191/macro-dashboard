...
def compute_speaker_forward_pe(
    ...
    *,
    ...
    selected_year = next_year if as_of.month >= switch_month else current_year
    ...
    horizon_coverage_ratio = round(next_mc / total_market_cap, 4) if total_market_cap > 0 else 0.0

    usable_constituent_count = sum(
        1 for c in constituents if c.price is not None and c.shares is not None
    )
    selected_year_complete = (
        speaker_pe is not None
        and usable_constituent_count > 0
        and coverage_count == usable_constituent_count
        and abs(coverage_ratio - 1.0) < 1e-9
    )

    signal_mode: SignalMode = "actionable" if selected_year_complete else "directional_only"
    ...
    if not valid:
        note = "Mag 7 speaker forward P/E unavailable — selected year has no usable constituent earnings."
    elif signal_mode == "directional_only":
        note = (
            "Mag 7 speaker forward P/E available, but selected-year constituent coverage is incomplete; "
            "publish as directional_only."
        )
    else:
        note = "Mag 7 speaker forward P/E actionable on complete selected-year constituent set."
