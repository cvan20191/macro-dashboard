...
def _defensive_override_needed(
    ...
    trap_confirmed_clear = stag.trap.trap_state == "false"
    if cb.quadrant == "A" and val.can_support_buy_zone and trap_confirmed_clear:
        return False
    ...
    if cb.quadrant in {"B", "C"} and trap_confirmed_clear:
        return False
...
def _derive_existing_positions_action(
    cb: ChessboardResult,
    val: ValuationResult,
    stag: StagflationResult,
    stress: StressResult,
    ...
    if _defensive_override_needed(cb, val, stag, stress):
        return "trim"
    if cb.quadrant == "D" and stress.stress_warning_active:
        return "hold_with_tighter_risk"
    if val.can_pause_new_buying:
        return "hold"
    if cb.quadrant == "A" and stag.trap.trap_state == "false":
        return "hold"
    return "hold_with_tighter_risk"
...
def _derive_reentry_condition(val: ValuationResult, cb: ChessboardResult) -> str | None:
    if val.can_pause_new_buying:
        return "Re-enter once true forward valuation compresses back into the 20-25x zone or liquidity improves further."
    if cb.quadrant == "D":
        return "Re-enter when balance-sheet contraction eases and rate pressure stops worsening."
    return None


def _derive_exit_condition(val: ValuationResult, cb: ChessboardResult, stag: StagflationResult) -> str | None:
    if cb.quadrant == "A":
        return "Exit or trim on a clear regime break: rates re-accelerate higher, balance-sheet support fades, or the trap reactivates."
    if stag.trap.active:
        return "Exit weaker cyclicals if sticky inflation persists while labor weakens further."
    if val.can_pause_new_buying:
        return "Avoid adding fresh risk until valuation is supported by true forward earnings."
    return None
