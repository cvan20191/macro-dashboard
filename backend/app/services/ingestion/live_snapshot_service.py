...
def _missing_cache_critical_fields(snapshot: IndicatorSnapshot) -> list[str]:
    ...
    if snapshot.inflation.core_cpi_yoy is None:
        missing.append("inflation.core_cpi_yoy")
    if snapshot.growth.pmi_manufacturing is None:
        missing.append("growth.pmi_manufacturing")
    return missing
...
_FRED_FETCH_MAP: dict[str, tuple[str, str, int, int]] = {
    ...
    "credit_card_chargeoff": (
        FRED_SERIES["credit_card_chargeoff"],
        "Credit Card Charge-Off Rate",
        100,
        800,
    ),
    "equity_market_value_z1": (
        FRED_SERIES["equity_market_value_z1"],
        "Z.1 Corporate Equities Market Value",
        100,
        800,
    ),
...
def _build_source_meta(raw: dict[str, FetchResult], statuses: dict[str, str]) -> dict[str, SourceMeta]:
    ...
    for key, result in raw.items():
        ...
        if source_class is None:
            if provider_label == "FMP":
                source_class = "licensed"
            elif provider_label == "Yahoo Finance":
                source_class = "proxy"
            elif provider_label == PROVIDER_STUB:
                source_class = "manual"
            else:
                source_class = "official"
        confidence_weight = result.confidence_weight
        if confidence_weight is None:
            confidence_weight = default_confidence_weight(source_class)
        meta[key] = SourceMeta(
            ...
            release_date=result.release_date or result.observed_at,
            last_revised_at=result.last_revised_at,
            staleness_bucket=result.staleness_bucket or statuses.get(key, result.status),
...
async def get_live_playbook(
    ...
    try:
        ...
        logger.warning("Macro expectations failed (non-fatal): %s", exc)
        macro = MacroExpectationsState(
            regime_impact_narrative=(
                "Macro expectations layer temporarily unavailable — durable regime "
                f"framework unchanged. ({exc})"
            ),
            tactical_posture_modifier="mixed — event risk elevated",
            generated_at=datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        )
    if playbook_conclusion is not None:
