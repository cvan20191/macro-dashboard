...
_FRED_HIST_MAP: dict[str, tuple[str, str, str, int]] = {
    ...
    "balance_sheet":     (FRED_SERIES["balance_sheet"],     "Fed Balance Sheet",            "weekly",    365),
    ...
    "npl_ratio":         (FRED_SERIES["npl_ratio"],         "Broad Bank Delinquency Rate", "quarterly", 800),
    "cre_delinquency":   (FRED_SERIES["cre_delinquency"],   "CRE Delinquency Rate", "quarterly", 800),
...
def _build_historical_source_meta(
    ...
    for key, result in raw.items():
        ...
        meta[key] = SourceMeta(
            ...
            note=result.note,
            source_class=result.source_class or ("proxy" if provider_label == "Yahoo Finance" else "official"),
            confidence_weight=result.confidence_weight or default_confidence_weight(result.source_class or ("proxy" if provider_label == "Yahoo Finance" else "official")),
            release_date=result.release_date or result.observed_at,
            last_revised_at=result.last_revised_at,
            staleness_bucket=result.staleness_bucket or "historical",
