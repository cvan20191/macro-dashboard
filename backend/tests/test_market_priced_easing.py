...
def test_free_backdrop_does_not_mark_100bps_as_stretch_by_itself() -> None:
    ...
    assert result.easing.expected_cut_bps_rest_of_year == 100.0
    assert result.easing.expected_cut_count_rest_of_year == 4.0
    assert result.easing.pricing_stretch_active is False


def test_unsorted_meetings_still_compute_correct_rest_of_year_cuts() -> None:
    snapshot = {
        "as_of": "2025-09-30",
        "source_mode": "manual_snapshot",
        "current_target_mid": 4.375,
    ...
    assert result.easing.expected_cut_bps_rest_of_year == 50.0
    assert result.easing.expected_cut_count_rest_of_year == 2.0


def test_next_year_meetings_do_not_distort_rest_of_year_read() -> None:
    snapshot = {
        "as_of": "2025-09-30",
        "source_mode": "manual_snapshot",
        "current_target_mid": 4.375,
...
def test_january_2026_snapshot_reads_cuts_for_2026_calendar_year() -> None:
    snapshot = {
        "as_of": "2026-01-10",
        "source_mode": "manual_snapshot",
        "current_target_mid": 4.375,
...
def test_missing_meeting_dates_make_year_end_read_unavailable() -> None:
    snapshot = {
        "as_of": "2025-09-30",
        "source_mode": "manual_snapshot",
        "current_target_mid": 4.375,
    ...
    assert result.easing.expected_cut_bps_rest_of_year is None
    assert result.easing.hard_actionable is False
    assert "year-end read is unavailable" in (result.easing.note or "")
