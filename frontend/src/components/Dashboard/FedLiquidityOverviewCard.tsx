...
import { fetchFedLiquidityOverview } from '../../api/playbook'
...
import type { FedLiquidityOverviewResponse } from '../../types/playbook'
...
function getFallbackOverview(): OverviewData {
  return data
}
    ...
    <div style={s.leverBlock}>
      <div style={s.leverHeader}>
        ...
        <div style={s.rightMeta}>
          <div>
            <strong>Latest:</strong> {lever.latest_value != null ? latestFormatter(lever.latest_value) : 'N/A'} ({lever.latest_date})
          </div>
          <div>
...
export function FedLiquidityOverviewCard({
  ...
  const [overview, setOverview] = useState<OverviewData>(getFallbackOverview())
  ...
  useEffect(() => {
    ...
    fetchFedLiquidityOverview(false)
      .then((payload: FedLiquidityOverviewResponse) => {
        if (!active) return
        setOverview(payload)
        setIsLiveFallback(false)
      })
  ...
  const anchorMs = useMemo(
    () => Math.max(endOfHistoryMs(overview.fed_balance_sheet), endOfHistoryMs(overview.fed_rate)),
    [overview],
  )

  const syncedView = useMemo(() => {
    const bsH = overview.fed_balance_sheet.history
    const rtH = overview.fed_rate.history
    if (rangeShared === 'All') {
      const firstBs = bsH.length ? parseDateMs(bsH[0].date) : parseDateMs(overview.fed_balance_sheet.latest_date)
      const firstRt = rtH.length ? parseDateMs(rtH[0].date) : parseDateMs(overview.fed_rate.latest_date)
  ...
  const asyncBsView = useMemo(() => {
    const anchor = endOfHistoryMs(overview.fed_balance_sheet)
    const pts = filterByRange(overview.fed_balance_sheet.history, rangeBs, anchor)
    return {
      points: pts,
      fromDate: pts[0]?.date ?? overview.fed_balance_sheet.latest_date,
      toDate: pts[pts.length - 1]?.date ?? overview.fed_balance_sheet.latest_date,
    }
  }, [rangeBs, overview])

  const asyncRateView = useMemo(() => {
    const anchor = endOfHistoryMs(overview.fed_rate)
    const pts = filterByRange(overview.fed_rate.history, rangeRate, anchor)
    return {
      points: pts,
      fromDate: pts[0]?.date ?? overview.fed_rate.latest_date,
      toDate: pts[pts.length - 1]?.date ?? overview.fed_rate.latest_date,
    }
  }, [rangeRate, overview])
  ...
  return (
    <Card title="Fed Liquidity Overview" accent="var(--blue)">
      ...
      {mode === 'live' && isLiveFallback && (
        <div style={s.fallbackNote}>
          Live liquidity endpoint unavailable; showing bundled fallback snapshot.
        </div>
      )}
      ...
      >
        {syncTimeline && syncHover !== null && bsAtHover && rateAtHover && (
          ...
          >
            ...
            <div style={s.syncTooltipLine}>
              <strong>{overview.fed_balance_sheet.label}:</strong> {formatBalanceSheet(bsAtHover.value)}{' '}
              <span style={s.syncAsOf}>(as of {bsAtHover.date})</span>
            </div>
            <div style={s.syncTooltipLine}>
              <strong>{overview.fed_rate.label}:</strong> {formatRate(rateAtHover.value)}{' '}
        ...
        <div style={s.stack}>
          <LeverBlock
            lever={overview.fed_balance_sheet}
