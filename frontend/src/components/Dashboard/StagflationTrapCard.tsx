  ...
  return (
    <Card title="Stagflation Trap Monitor">
      ...
      {!isActive && (
        <div style={{
          padding: '6px 10px',
          background: trap.inflation_inputs_incomplete ? 'var(--yellow-bg)' : 'var(--green-bg)',
          border: `1px solid ${trap.inflation_inputs_incomplete ? 'var(--yellow-dim)' : 'var(--green-dim)'}`,
          borderRadius: 'var(--radius-sm)',
          fontSize: '12px',
          color: trap.inflation_inputs_incomplete ? 'var(--yellow)' : 'var(--green)',
        }}>
          {trap.trap_assessment_note ?? 'Stagflation trap not currently active'}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        ...
        <StatusChip
          label={
            trap.inflation_inputs_incomplete
              ? 'Inflation incomplete'
              : trap.sticky_inflation
                ? 'Sticky inflation'
                : 'Not sticky (confirmed)'
          }
          active={!!trap.inflation_inputs_incomplete || trap.sticky_inflation}
          color={trap.inflation_inputs_incomplete ? 'var(--yellow)' : 'var(--red)'}
        />
        ...
        <div style={s.side}>
          <div style={s.sideHeader}>
            ...
            <span style={{
              ...
              {trap.inflation_headline ?? (trap.sticky_inflation ? 'Sticky' : 'Inflation confirmation incomplete')}
            </span>
          </div>
