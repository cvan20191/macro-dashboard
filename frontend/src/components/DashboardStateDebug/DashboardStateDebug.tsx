    ...
    <div style={styles.container}>
      ...
      {open && (
        <div style={styles.grid}>
          ...
          />
          <DebugRow
            label="Card charge-off zone"
            value={`${state.systemic_stress?.credit_card_chargeoff_zone ?? '—'} (${state.systemic_stress?.credit_card_chargeoff_rate?.toFixed(2) ?? '—'}%)`}
          />
          <DebugRow
            label="Equity / M2"
            value={`${state.systemic_stress?.market_cap_m2_zone ?? '—'} (${state.systemic_stress?.market_cap_m2_ratio?.toFixed(2) ?? '—'}) [${state.systemic_stress?.equity_m2_ratio_source ?? '—'}]`}
          />
          <DebugRow
            label="Equity numerator as-of"
            value={state.systemic_stress?.equity_m2_numerator_as_of ?? '—'}
          />
          <DebugRow
            label="Equity numerator freshness"
            value={state.systemic_stress?.equity_m2_numerator_freshness ?? '—'}
          />
