...
export interface PlaybookData {
  ...
  stale_series?: string[]
  generated_at?: string
  macro_expectations?: LivePlaybookResponse['macro_expectations']
}
...
const MOCK_SNAPSHOT: IndicatorSnapshot = {
  ...
  systemic_stress: {
    yield_curve_10y_2y: -0.42,
    npl_ratio: 1.1,
    credit_card_chargeoff_rate: 3.9,
    market_cap_m2_ratio: 2.35,
    equity_m2_ratio_source: 'spy_fallback',
    equity_m2_numerator_as_of: '2026-02-12',
...
const DOCTRINE_FIXTURE_A_BUYZONE_STRESS: IndicatorSnapshot = {
  ...
  systemic_stress: {
    yield_curve_10y_2y: 0.56,
    npl_ratio: 1.57,
    credit_card_chargeoff_rate: 5.6,
    market_cap_m2_ratio: 2.51,
    equity_m2_ratio_source: 'spy_fallback',
    equity_m2_numerator_as_of: '2026-03-28',
    equity_m2_numerator_freshness: 'fresh',
  },
...
export function usePlaybookData(): UsePlaybookDataReturn {
    ...
    try {
      if (currentMode === 'live') {
        ...
        setData({
          ...
          macro_expectations: live.macro_expectations,
        })
      } else {
  ...
  const deterministicSummary = useMemo(() => state?.deterministic_summary ?? null, [state])
