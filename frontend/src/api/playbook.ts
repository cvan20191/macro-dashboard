...
export async function fetchFedLiquidityOverview(
  forceRefresh = false,
): Promise<FedLiquidityOverviewResponse> {
  const url = `${BASE_URL}/api/live/fed-liquidity-overview${forceRefresh ? '?force_refresh=true' : ''}`
  const response = await fetch(url)
  if (!response.ok) {
    const text = await response.text().catch(() => response.statusText)
    throw new Error(`Fed liquidity overview request failed (HTTP ${response.status}): ${text}`)
  }
  return response.json() as Promise<FedLiquidityOverviewResponse>
}
